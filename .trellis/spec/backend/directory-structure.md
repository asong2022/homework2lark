...
│   │   ├── ocr/                     # Fake and PaddleOCR adapters
│   │   ├── publication/             # Fake and lark-cli reviewed-problem publishers
...
- Domain code owns review eligibility, status transitions, immutable-version rules, and stable domain errors.
...
- Add a service method for a complete use case such as upload, create region, run OCR, revise, review, or read normalized record.
...
- IDs use semantic prefixes (`asset_`, `region_`, `ocr_`, `revision_`, `problem_`, `review_event_`) followed by UUID text.
