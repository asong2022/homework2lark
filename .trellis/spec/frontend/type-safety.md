...
- Discriminated unions for request state and review status.
- Exact `NormalizedProblemRecord`, OCR block/run, revision, lineage, asset, region, and error envelope shapes.
...
- A generated TypeScript declaration is not runtime validation. Success decoders must validate nested region bbox/media URLs, review events, lineage IDs, OCR/revision histories, and timestamps before a component receives the record. A malformed 2xx becomes `invalid_response`, not a render crash.

## Forbidden Patterns

- `any`, double assertions (`as unknown as X`), non-null assertions used to bypass workflow state, or local casts of raw `fetch().json()`.
- Duplicating review-status strings in several components; export one `ReviewStatus` union.
