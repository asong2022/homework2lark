...
{
  "pages": [
    {"pageNumber": 1, "path": "private-inputs/blank-P1.png"},
    {"pageNumber": 2, "path": "private-inputs/blank-P2.png"}
  ]
}
```
...
```powershell
python <skill-root>/scripts/staged_intake.py start `
  --roster <私有名单.json> `
  ...
  "submissions": [
    {
      "studentNumber": "01",
      "pages": [
        {"pageNumber": 1, "path": "private-inputs/01-P1.jpg"},
        {"pageNumber": 2, "path": "private-inputs/01-P2.jpg"}
      ]
    }
  ]
}
...
```powershell
python <skill-root>/scripts/staged_intake.py add `
  --campaign-dir <活动目录> `
  --input <本批页面清单.json>
```
...
```powershell
python <skill-root>/scripts/staged_intake.py complete `
  --campaign-dir <活动目录> `
  --batch-id B01 `
  --input <本批观察结果.json>
```
...
```powershell
python <skill-root>/scripts/staged_intake.py status --campaign-dir <活动目录>
python <skill-root>/scripts/staged_intake.py export --campaign-dir <活动目录> --output <新的汇总.json>
```
