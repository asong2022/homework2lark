...
1. 经过教师确认修订的数学题目是最小可复用教学资产。
2. OCR 是不可覆盖的机器证据，不是题目真值。
3. 原始、机器派生、人工修订三层分离。
4. `ProblemAsset` 表示稳定题目与当前人工修订指针；Base `MistakeGroupRecord` 汇总同题同类错误学生，未来原子级 `MistakeOccurrence` 表示单个学生的一次错误事实，三者禁止混为一体。
...
一次自动框题产生一个不可覆盖的 `RegionDetectionRun`，其下每个 `RegionCandidate` 是 Provider 返回的一条题目级建议。Yescan 中它与一个 group-level `StructureInfo` 一一对应；题内文字、公式、图形、表格或示意图 `Detail` 不会再拆成候选。Provider 仍可能误合并或误切，因此候选不是可复用题目资产。
...
| `corrected_text` | 非空文本 | 教师确认后的完整题目内容 |
...
题目资产聚合入口。它只保存稳定题目 ID 和当前人工修订指针，不承担审核状态机职责。

| 字段 | 类型/约束 | 职责 |
|---|---|---|
| `id` | `problem_` UUID，PK | 对外 `problemId` |
| `problem_region_id` | FK，唯一 | 一区域对应一个当前题目资产 |
| `current_revision_id` | FK，可空 | 最近保存且由教师确认的人工版本 |
| `created_at`/`updated_at` | UTC | 生命周期时间 |

保存有效修订后立即更新 `current_revision_id`。发布资格由当前修订、来源、裁图和 OCR 依据是否完整决定，不存储 `review_status`、`reviewed_at` 或派生复用布尔值。
...
ProblemRegion 1 ── 1 ProblemAsset
ProblemRevision 0..1 ← current ── ProblemAsset
ProblemAsset 1 ── 0..1 ProblemPublication

Future only:
ProblemAsset 1 ── N MistakeOccurrence

Current Base projection only:
ProblemAsset 1 ── N MistakeGroupRecord ── N student labels
```
...
→ 教师确认完整题目
→ 新增 ProblemRevision
→ 更新 ProblemAsset.current_revision_id
→ 明确发布到飞书
```

OCR 重试不改变当前人工修订。保存新修订会立即切换当前版本；飞书发布按稳定题目 ID 和本地修订版本幂等同步。发布失败保留全部本地证据，只重试发布步骤。
...
- `MistakeOccurrence`：学生/班级在某时某来源下的一次错误事实，关联 `ProblemAsset.id`。
...
- ProblemRevision ≠ ProblemAsset：修订是不可变版本，题目资产保存当前版本指针。
- ProblemAsset ≠ MistakeOccurrence：题目资产不是某个学生的一次错误。
- 题目学情 lookup ≠ MistakeOccurrence：聚合视图不能承担逐人、逐次作答历史。
- MistakeGroupRecord ≠ MistakeOccurrence：Base 分组行可包含多名同类错误学生，不是一个学生一行的原子事实。
- 当前人工修订 ≠ OCR 成功：教师确认的内容门槛不可由 Provider 绕过。
